...
int fillEndpointList(char* cmd_str, int len, int* endpoint_list); //fill the endpoint list
...
void setup()
{
  ...
  hostn = location.concat(hostn); //put location.local
...
void loop()
{
  ...
  if(client)
  {
    ...
    if(command.length() > 0){
      ...
      n_endpoints = fillEndpointList(command.c_str(), command.length(), endpoints);
      ...
      if(opt_value == "text")
      {
        ...
        String txt_value = command.substring(txt_index,nextEndpoint(txt_index,endpoints,10));

        Serial.println(command);
        Serial.print("txt_indexes: ");
        Serial.print(txt_index_start,DEC);
        Serial.print("-");
        Serial.println(txt_index_start,DEC);
        Serial.print("text=");
        ...
        dmd.drawString(0, 0, F(txt_value.c_str()));
  ...
  if (clockMode)
  {
    ...
    if (current_runtime - last_runtime > refresh_runtime)
    {
      ...
      http.collectHeaders(important_headers, 1);      
...
*/
        ...
        //TODO
        //dmd.drawString(0, 0, F(datetime.c_str()));
        Serial.println()
        }
      }
      else 
...
int fillEndpointList(char* cmd_str, int len, int* endpoint_list){
  int ep = 0;
  int i;
  for(i = 0; i < len; i++){
